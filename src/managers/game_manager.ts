...
import {
  ...
  CheckTurn,
  CheckVictory,
} from "../engine/preconditionners/logic_cond";
...
class GameManager implements EngineObject {
  ...
  constructor(
    ...
    this.__rule = rule;
    if (tile_manager) this.__tiles_manager = tile_manager;
    else
      this.__tiles_manager = new TilesManager(
        this.board,
        this.rule.initial_tiles(this),
      );
  ...
  public nextTurn(): boolean {
    ...
    this.__current_turn =
      this.__current_turn == Color.BLUE ? Color.RED : Color.BLUE;
