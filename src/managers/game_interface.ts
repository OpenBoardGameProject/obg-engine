...
export class GameInterface implements EngineObject {
  ...
  get tiles(): Tile[] {
    ...
    const our_objects = this.__game.tiles_manager.tiles.filter(
      (tile) => tile.object?.color == this.__player.color,
    );
    const visibleTiles: Vector2D[] = our_objects.flatMap((tile) =>
      this.__game.tiles_manager.visibleTiles(
        tile.pos,
        tile.object?.config.properties.vision.range ?? 0,
      ),
    );
    ...
    tiles = tiles.fill(
      EMPTY_TILE,
      0,
      width * this.__game.board.config.properties.height,
    );
